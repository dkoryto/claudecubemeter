...
// LCD configuration defaults for SmallTV-Ultra (use 4 for HelloCubic Lite)
...
class ConfigManager {
   ...
   public:
    ...
    const char* getAnthropicToken() const { return anthropic_token.c_str(); }
    void setAnthropicToken(const char* t) {
        if (t) anthropic_token = t;
    }
    const char* getWebhookUrl() const { return webhook_url.c_str(); }
    void setWebhookUrl(const char* u) {
        if (u) webhook_url = u;
    }
};
