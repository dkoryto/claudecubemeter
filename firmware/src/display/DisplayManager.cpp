...
auto DisplayManager::drawStartup(String currentIP) -> void {
    ...
    // Big title centered: "ClaudeCubeMeter" size 2 (15 chars * 12 = 180 px)
    const int16_t titleW = 15 * 12;
    g_lcd.setTextSize(2);
    g_lcd.setTextColor(LCD_WHITE, LCD_BLACK);
    g_lcd.setCursor((LCD_W - titleW) / 2, 30);
    g_lcd.print("ClaudeCubeMeter");

    // Subtitle line 1, size 1 centered
    const char* sub1 = "Claude Cube Meter";
    g_lcd.setTextSize(1);
    g_lcd.setTextColor(0xBDF7, LCD_BLACK);
    int16_t sub1_w = static_cast<int16_t>(strlen(sub1)) * 6;
    g_lcd.setCursor((LCD_W - sub1_w) / 2, 60);
    g_lcd.print(sub1);

    // Subtitle line 2, author
    const char* sub2 = "by Dariusz Koryto";
    int16_t sub2_w = static_cast<int16_t>(strlen(sub2)) * 6;
    g_lcd.setCursor((LCD_W - sub2_w) / 2, 75);
    g_lcd.print(sub2);

    // Version
    String ver = String(PROJECT_VER_STR);
    int16_t ver_w = static_cast<int16_t>(ver.length()) * 6;
    g_lcd.setTextColor(0x8410, LCD_BLACK);
    g_lcd.setCursor((LCD_W - ver_w) / 2, 100);
    g_lcd.print(ver);

    // IP
    String ip = String("IP: ") + currentIP;
    int16_t ip_w = static_cast<int16_t>(ip.length()) * 6 * 2;
    g_lcd.setTextSize(2);
    g_lcd.setTextColor(LCD_WHITE, LCD_BLACK);
    g_lcd.setCursor((LCD_W - ip_w) / 2, 130);
    g_lcd.print(ip);

    // RGB squares at bottom (kept as connectivity indicator)
    const int16_t box = 36;
    const int16_t gap = 16;
    const int16_t boxY = 180;
    const int16_t totalW = box * 3 + gap * 2;
    const int16_t boxX = (LCD_W - totalW) / 2;
    g_lcd.fillRect(boxX, boxY, box, box, LCD_RED);
    g_lcd.fillRect(boxX + box + gap, boxY, box, box, LCD_GREEN);
    g_lcd.fillRect(boxX + (box + gap) * 2, boxY, box, box, LCD_BLUE);
